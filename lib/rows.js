...
async function listRows(log, tableName, partitionKey, conditions) {
    ...
    if (conditions) {
        conditions.forEach(([condition, arguments], index) => {
            if (index === 0)
                query = query.where(condition, arguments);
            else
                query = console.log(condition, arguments) || query.and(condition, arguments);
        });
    }
...
module.exports = { createRow, upsertRow, listRows, fetchRow, deleteRow };