...
const requireAnyUser = require("../lib/require-any-user");
...
app.get("/api/documents/users/me", requireAnyUser, or500(async (req, res) => {
...
app.post("/api/documents/users/me", requireAnyUser, or500(async (req, res) => {

    const { context, user, body } = req;
    const { tenant: tid } = body;
    if (!tid) {

        res.status(400).send("Tenant not specified");
...
app.patch("/api/documents/users/me/:id", requireAnyUser, or500(async (req, res) => {
